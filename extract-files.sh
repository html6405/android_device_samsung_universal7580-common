...
"${PATCHELF}" --replace-needed libutils.so libutils-v32.so $BLOB_ROOT/lib/libexynosdisplay.so
"${PATCHELF}" --replace-needed libutils.so libutils-v32.so $BLOB_ROOT/lib/hw/hwcomposer.exynos5.so
